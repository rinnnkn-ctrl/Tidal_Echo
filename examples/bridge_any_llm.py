...
convo: collections.deque = collections.deque(maxlen=max(HISTORY_N * 2, 8))
...
def execute_code(language: str, code: str, stdin: str = "") -> str:
    ...
    try:
        ...
        runner_map = {
            "js": "node", "sh": "bash", "rb": "ruby",
            "go": "go run", "rs": "rustc -o /tmp/prog && /tmp/prog",
        }
...
def install_package(package: str) -> str:
    try:
        ...
        execution = sb.run_code(
            f"import subprocess; r = subprocess.run(['pip', 'install', '{package}', '-q'], "
            f"capture_output=True, text=True); print(r.stdout[-500:] if r.stdout else '')"
        )
...
TOOLS = [
    {
        ...
        "function": {
            "name": "execute_code",
            "description": "在云端沙盒里执行代码并返回输出。沙盒有网络，可以import包、爬数据、调API。支持python/javascript/bash/ruby/go/rust。同一次对话共用同一个沙盒，变量和文件会保留。",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "在沙盒里 pip install 一个 Python 包。",
            "parameters": {
                "type": "object",
                "properties": {
